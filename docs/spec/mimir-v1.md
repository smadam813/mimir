...
1. **Wisdom browser + curation**: the selected Project's **ambient candidate universe** — its Wisdom plus Global, Retired excluded (ADR-0009) — narrowed by search and by kind; the sidebar reaches contested, orphaned and Retired. Detail with text, versions, reinforcement, Provenance (drill to the underlying Events); actions: **edit** (new WisdomVersion, `cause=edited`, re-embed), **Retire/unretire**, **Delete** (confirmed; removes the Wisdom + version chain).
