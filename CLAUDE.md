...
- `appsettings.json` restates the §11 defaults baked into the `Configuration/` options classes, and `AppSettingsTests` sweeps every options class it finds by convention: a changed value, an omitted key and a key with no knob behind it all fail. So a default changes in both places, and a new knob or section ships in both.
