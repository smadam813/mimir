...
A new §11 section is one options class plus one `AddSection` line in `MimirOptionsRegistration`, and nothing else. `ValidateOnStart()` on that call is what makes a bad knob fail the boot rather than surfacing at first access; dropping it leaves every options test green, because they all validate on access ([#140](https://github.com/smadam813/mimir/issues/140)'s second item).
