...
A new §11 section is one options class, one `AddSection` line in `MimirOptionsRegistration`, and its block in `appsettings.json` spelling out every knob at the default the class already carries. The last of the three is enforced — `AppSettingsTests` discovers sections by their `SectionName` const, so the block is demanded the day the class lands. The `AddSection` line is not, and cannot easily be: `IOptions<T>` resolves whether or not the section was ever registered, so a forgotten line binds nothing, validates nothing, and fails nowhere.
